...
`Directory.Build.props` supplies version 0.1.1 to the WPF, CLI, Core, and Infrastructure projects, including assembly, file, and informational metadata. User-visible version strings are resolved from the Core assembly metadata so the main-window title/status bars and headless output cannot drift from the built components.
