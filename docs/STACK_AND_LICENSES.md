...
- **CatClipComposer.Plugins.BuiltIn assembly 0.1.35:** First-party dynamic module assembly containing
...
`Directory.Build.props` supplies version 0.1.35 to the WPF, CLI, Core, Infrastructure, and built-in plugin
projects, including
...
The repository must also contain exactly one extensionless `version_<version>` marker matching that property.
...
Normal tagged releases bundle application assemblies and native SQLite into two framework-dependent single-file
entry points and therefore require Microsoft's .NET 8 Desktop Runtime x64, whose Windows distribution remains
outside the Cat Clip Composer archive. An explicit self-contained publisher mode also embeds the runtime in the
two compressed executables. FFmpeg, plugins, and fonts remain external subfolders in both modes. Neither mode
changes application or dependency licenses.
