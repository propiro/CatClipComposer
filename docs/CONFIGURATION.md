...
`PreviewSlideCount` is clamped to 1-24 and defaults to 12. `RescanLibraryOnStartup` defaults to true; startup
skips scanning when no source folder is configured. Source folders use zero-based `FolderN` keys, load in
numeric order, and are deduplicated case-insensitively.
